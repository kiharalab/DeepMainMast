...
bool readmrc(MRC *mrc, char *filename){
 ...
 fseek(fpin,4*(256-54)+nsymbt,SEEK_CUR);
 	...
 	mrc->widthx = mrc->xlen / (float) mrc->mx;
        mrc->widthy = mrc->ylen / (float) mrc->my;
        mrc->widthz = mrc->zlen / (float) mrc->mz;
...
//Just x2x2x2
bool upsampling(MRC *mrc,float t){
 int i,j,k,ind1,ind2,ind3;
 int Nth=omp_get_max_threads();
 float rgstep=1.000/mrc->widthx;
 float *map;
 unsigned int cnt=0;
 //Filtering Only
 if(mrc->widthx < 1.0){
...
bool fastLDP(MRC *m,POINTS *p){
 ...
 //malloc
 if((p->cd=(float **)malloc(sizeof(float *)*m->NumVoxels))==NULL)
  return true;
 if((p->origrid=(int **)malloc(sizeof(int *)*m->NumVoxels))==NULL)
  return true;

 for(i=0;i<m->NumVoxels;i++)
  if((p->cd[i]=(float *)malloc(sizeof(float)*3))==NULL)
    return true;
 ...
 for(int z=0;z<m->zdim;z++){
  ...
  if(m->dens[ind]==0.00)
   continue;
   //if((p->cd[cnt]=(float *)malloc(sizeof(float)*3))==NULL)
   // return true;
 ...
 puts("#Start LDP");
 if((p->dens=(float *)malloc(sizeof(float)*cnt))==NULL)
  return true;
 //Setup Filter
 //Gaussian kernel dreso=window size
 float dreso=cmd.dreso;
 float gstep=m->widthx;
 float fs=(dreso/gstep)*0.5;
 fs=fs*fs;
 float fsiv=1.000/fs;
 float fmaxd=(dreso/gstep)*2.0;
 printf("#maxd= %f\n",fmaxd);
 //Mean Shifting
...
bool fastVEC(MRC *m,MRC *M){
 ...
 if((M->vec=(float **)malloc(sizeof(float *)*Ndata))==NULL)
  return true;
 if((M->dens=(float *)calloc(sizeof(float),Ndata))==NULL)
  return true;

 if((M->sco=(float *)calloc(sizeof(float),Ndata))==NULL)
  return true;

 for(i=0;i<Ndata;i++){
  //if((M->vec[i]=(float *)malloc(sizeof(float)*3))==NULL)
  if((M->vec[i]=(float *)calloc(3,sizeof(float)))==NULL)
  return true;
 }
 ...
 fs=fs*fs;
 float fsiv=1.000/fs;
 float fmaxd=(dreso/gstep)*1.5;//Padding
 printf("#maxd= %f\n",fmaxd);
 ...
 //50,000 tbl
 float *exp_tbl;
 if((exp_tbl=(float*)calloc(sizeof(float),50000))==NULL)
	 return true;
 for(int i=0;i<50000;i++){
	 //printf("%f %f\n",-0.00100*i,exp((float)(-0.00100*i)));
	 exp_tbl[i]=exp((float)(-0.00100*i));
 }
 #pragma omp parallel for schedule(dynamic,5)
 for(int x=0;x<M->xdim;x++){
  float rx,ry,rz,d2;
  ...
  if((int)pos[0]<0 || (int)pos[0]>=m->xdim)
 	...
 	for(int y=0;y<M->ydim;y++){
 	 ...
 	 if((int)pos[1]<0 || (int)pos[1]>=m->ydim)
  	 ...
  	 //printf("x= %d %d %d\n",x,y,z);
		 ...
		 if(endp[1]>m->ydim) endp[1]=m->ydim;
   		 endp[2]=(int)(pos[2]+fmaxd+1);
		 if(endp[2]>m->xdim) endp[2]=m->zdim;
   ...
   for(int zp=stp[2];zp<endp[2];zp++){
    ...
    ind2=xydim*zp+m->xdim*yp+xp;
    //v=exp(-1.50*d2*fsiv)*m->dens[ind2];
    //printf("d= %d\n",(int)(1.500*d2*fsiv*1000.00));
    if((int)(1.500*d2*fsiv*1000.00) > 50000-1)
     v=0.00;
    else
     v=exp_tbl[(int)(1.500*d2*fsiv*1000.00)]*m->dens[ind2];
    //v=m->dens[ind2];
    dtotal+=v;
    //if(v>0)
    //printf("d %d %d %d %d exp(k)= %f w= %f\n",(int)(1.50*d2*fsiv*1000.00),xp,yp,zp,exp_tbl[(int)(1.50*d2*fsiv*1000.00)],exp(-1.50*d2*fsiv));
    pos2[0]+=v*(float)xp;
    pos2[1]+=v*(float)yp;
    pos2[2]+=v*(float)zp;
   }}}
   ...
   float dvec=sqrt(tmpcd[0]*tmpcd[0]+tmpcd[1]*tmpcd[1]+tmpcd[2]*tmpcd[2]);
   if(dvec==0.000)
	   dvec=1.000;
   //Unit Vector
   float rdvec=1.000/dvec;
 ...
 M->std=sqrt(dsum);
 printf("#MAP AVE= %f STD= %f STD_norm= %f SUM= %f\n",M->ave,M->std,M->std_norm_ave,M->dsum);
 return false;
}
...
float meanshift_pos(MRC *m,float pos[3]){
 int i,j,k,ind;
 int cnt=0;
 int xydim=m->xdim*m->ydim;
 POINTS *p;
...
bool MergePoints(MRC *m,POINTS *p){
 float dcut=cmd.MergeDist/m->widthx;
 float d2cut=dcut*dcut;
 float rdcut=cmd.Filter;
 float dmax,dmin,drange;
 bool *stock;
...
void ShowPath(MRC *m,POINTS *p,GRAPH *g, TREE *t, int n){
 ...
 for(int mid=0;mid<n;mid++){
  float score=QualityTree(g, &t[mid]);
  printf("#SCORE: %f\n",score);
  printf("MODEL %d\n",mid+1);
	Natm=1;
	now=t[mid].St;
  ...
  //get_path(g, &t[mid],st,ed);
	...
	for(int j=0;j<p->Nori;j++){
	 ...
	 if(p->mask[j]==0.00||m->dens[ind]==0.00)
	  continue;
  	 //map origin xyz and xwidth based coordinates
  	 tmp[0]=(float)x; tmp[1]=(float)y; tmp[2]=(float)z;
...
bool SetUpGraph(POINTS *p, GRAPH *g,MRC *mrc,TREE *mst){
 int i,j,dif;
 float dcut=cmd.LocalR/mrc->widthx;
 float d2cut=dcut*dcut;
 int *cid;
 ...
 for(int ii=0;ii<g->Ne;ii++){
  float cd1[3],vec[3],dens,MinDens;
  ...
  for(int jj=1;jj<11;jj++){
   cd1[0]=p->cd[v1][0]+vec[0]*0.1*(float)(jj);
   cd1[1]=p->cd[v1][1]+vec[1]*0.1*(float)(jj);
   cd1[2]=p->cd[v1][2]+vec[2]*0.1*(float)(jj);
 ...
 //End Local MST
 //Set Keep Flag
 float dkeep=cmd.Dkeep/mrc->widthx;
 for(int jj=0;jj<g->Ne;jj++){
...
bool Tabu(GRAPH *g,TREE *init_t,TREE *Tr){
 ...
 int Ntabu=cmd.Ntabu;
 int Nsim=cmd.Nsim;
 float AllowLen=init_t->len*cmd.Allow;
 MOVE *move;
 int tabu[1000];
 int Ntb=0;
 int tbp=0;
 ...
 //float score;
 float score=QualityTree(g,&Tnow);
...
bool InitTree(TREE *t,bool mode) {
 ...
 if(mode){
  ...
  if((t->cost=(float *)malloc(sizeof(float)*t->Nnode))==NULL)
   return true;
...
float QualityTree(GRAPH *g,TREE *t){
 int i,j;
...
bool CopyTree(TREE *in, TREE *out,bool mode){
 ...
 if(mode){
  ...
  if((out->cid=(int *)malloc(sizeof(int)*out->Nnode))==NULL)
   return true;
  if((out->cost=(float *)malloc(sizeof(float)*out->Nnode))==NULL)
   return true;
...
bool SetCutTbl(GRAPH *g,TREE *t,int *tabu,int Ntb,float LenCutoff){
 int i,j,n=0;
...
//Cut and..
int CutTree(GRAPH *g,TREE *t,float LenCutoff,int *tabu,int Ntb){
 int p,i,j,w,n=0;
 int Nst=0;
 int v,Nadd,cut_id;
 float len=t->len;
...
float RandDouble(){
 float a=rand();
 return (float)a/(float)(RAND_MAX+1.0);
}
...
float Kendall(int *a,int Na,int *b,int Nb){
 int i,j,k;
 int Ntrue=0;
 int Nfalse=0;
 bool f[5000]={false};
 ...
 //printf("Nt= %d Nf= %d\n",Ntrue,Nfalse);
 //return (float)(2*Ntrue)/(float)(Ntrue+Nfalse)-1.00;
 return (float)(2*Ntrue)/(float)(Nb*(Nb-1)*0.5)-1.00;
}
...
float OptPath2points(GRAPH *g,TREE *t,TREE *tmp,int Nbeam,int St,int Ed){
...
void SetUpVoxSize(MRC *m,MRC *M,float t,float ssize){
 int Ndata=m->xdim*m->ydim*m->zdim;
 int xydim=m->xdim*m->ydim;
 int xdim=m->xdim;
 ...
 for(int z=0;z<m->zdim;z++){
  ...
  d2=((float)x-cent[0])*((float)x-cent[0])
    +((float)y-cent[1])*((float)y-cent[1])
    +((float)z-cent[2])*((float)z-cent[2]);
  if(d2>dmax)
   dmax=d2;
 }}}
...
void SetUpComVoxSize(MRC *m1,MRC *m2,MRC *M1,MRC *M2,float t,float T,float ssize){
 int Ndata=m1->xdim*m1->ydim*m1->zdim;
 ...
 printf("BOUND:%d %d %d - %d %d %d\n",Xmax,Xmin,Ymax,Ymin,Zmax,Zmin);
 //Real Space
 float Bound[2][3];
 //Min
 Bound[0][0]=Xmin*m1->widthx+m1->orgxyz[0];
 Bound[0][1]=Ymin*m1->widthx+m1->orgxyz[1];
 Bound[0][2]=Zmin*m1->widthx+m1->orgxyz[2];
 //Max
 Bound[1][0]=Xmax*m1->widthx+m1->orgxyz[0];
 Bound[1][1]=Ymax*m1->widthx+m1->orgxyz[1];
 Bound[1][2]=Zmax*m1->widthx+m1->orgxyz[2];
 printf("BOUND m1:%.1f %.1f %.1f - %.1f %.1f %.1f\n",Bound[0][0],Bound[0][1],Bound[0][2],Bound[1][0],Bound[1][1],Bound[1][2]);
 
 //For m2
 xydim=m2->xdim*m2->ydim;
 xdim=m2->xdim;
 ...
 for(int y=0;y<m2->ydim;y++){
 for(int z=0;z<m2->zdim;z++){
  int ind=xydim*z+xdim*y+x;
  if(m2->dens[ind]<T){
   m2->dens[ind]=0.00;//filter
   continue;
  }
  if(Xmax<x) Xmax=x;
  if(Ymax<y) Ymax=y;
 ...
 //Common Min
 if(Bound[0][0] > Xmin*m2->widthx+m2->orgxyz[0])
  Bound[0][0] = Xmin*m2->widthx+m2->orgxyz[0];
 if(Bound[0][1] > Ymin*m2->widthx+m2->orgxyz[1])
  Bound[0][1] = Ymin*m2->widthx+m2->orgxyz[1];
 if(Bound[0][2] > Zmin*m2->widthx+m2->orgxyz[2])
  Bound[0][2] = Zmin*m2->widthx+m2->orgxyz[2];

 //Max
 if(Bound[1][0] < Xmax*m2->widthx+m2->orgxyz[0])
  Bound[1][0]=Xmax*m2->widthx+m2->orgxyz[0];
 if(Bound[1][1] < Ymax*m2->widthx+m2->orgxyz[1])
  Bound[1][1]=Ymax*m2->widthx+m2->orgxyz[1];
 if(Bound[1][2] < Zmax*m2->widthx+m2->orgxyz[2])
  Bound[1][2]=Zmax*m2->widthx+m2->orgxyz[2];
 
 printf("BOUND m1&m2:%.1f %.1f %.1f - %.1f %.1f %.1f\n",Bound[0][0],Bound[0][1],Bound[0][2],Bound[1][0],Bound[1][1],Bound[1][2]);

 cent[0] = 0.50*(Bound[0][0] + Bound[1][0]);
 cent[1] = 0.50*(Bound[0][1] + Bound[1][1]);
 cent[2] = 0.50*(Bound[0][2] + Bound[1][2]);
 ...
 M1->xdim=M2->xdim=(int)((Bound[1][0] - Bound[0][0])/ssize);
 M1->ydim=M2->ydim=(int)((Bound[1][1] - Bound[0][1])/ssize);
 M1->zdim=M2->zdim=(int)((Bound[1][2] - Bound[0][2])/ssize);
 
 M1->orgxyz[0]=M2->orgxyz[0]=Bound[0][0];
 M1->orgxyz[1]=M2->orgxyz[1]=Bound[0][1];
 M1->orgxyz[2]=M2->orgxyz[2]=Bound[0][2];

 
 printf("#COM Nvox= %d*%d*%d\n",M1->xdim,M1->ydim,M1->zdim);
 printf("#COM cent= %f %f %f\n",M1->cent[0],M1->cent[1],M1->cent[2]);
 printf("#COM ori= %f %f %f\n",M1->orgxyz[0],M1->orgxyz[1],M1->orgxyz[2]);
