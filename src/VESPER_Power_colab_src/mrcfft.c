...
//ZXY
void Euler2mtxZYX(float a,float b, float c,float m[3][3]){
...
void Euler2mtxXYZ(float a,float b, float c,float m[3][3]){
...
void mtxmtx(float a[3][3],float b[3][3],float c[3][3]){
 //puts("Product");
 for(int i=0;i<3;i++){
  for(int j=0;j<3;j++){
...
void rot_cd(float in[3],float out[3],float m[3][3]){
...
void inverse_mtx(float a[3][3],float b[3][3]){
 //float det=0;
/*
 int i,j;
 for(i=0;i<3;i++){
...
void RotMRC(MRC *in,MRC *out,float rx, float ry, float rz){

 int xydim=in->xdim*in->ydim;
 int xdim=in->ydim;
 int ind;
 float mtx[3][3],iv_mtx[3][3],tmp[3][3];
 float cent[3];

 cent[0]=0.5*(float)(in->xdim);
 cent[1]=0.5*(float)(in->ydim);
 cent[2]=0.5*(float)(in->zdim);
 ...
 //init
 for(int x=0;x<xdim*xdim*xdim;x++){
  out->vec[x][0]=0.0;
  out->vec[x][1]=0.0;
  out->vec[x][2]=0.0;
 }
 for(int x=0;x<in->xdim;x++){
  ...
  float vec[3],vec2[3];
  int nx,ny,nz,ind2;
  pos[0]=(float)x-cent[0];
 for(int y=0;y<in->ydim;y++){
  pos[1]=(float)y-cent[1];
 for(int z=0;z<in->zdim;z++){
  pos[2]=(float)z-cent[2];
...
void Q2mtx(float q[4],float mtx[3][3]){
...
//Q
void RotMRCbyQ(MRC *in,MRC *out,float q[4]){
...
void RotMRCbyQ2(MRC *in,MRC *ref,MRC *out,float q[4]){
 ...
 for(int z=0;z<in->zdim;z++){
  ...
  pos2[0]=pos2[0]*in->widthx+in->orgxyz[0]-ref->orgxyz[0];
  pos2[1]=pos2[1]*in->widthx+in->orgxyz[1]-ref->orgxyz[1];
  pos2[2]=pos2[2]*in->widthx+in->orgxyz[2]-ref->orgxyz[2];
	

  //ind2=xydim*(int)(pos2[2])+xdim*(int)pos2[1]+(int)pos2[0];
  ind2=ref->xdim*ref->zdim*(int)(pos2[2])+ref->xdim*(int)pos2[1]+(int)pos2[0];
  //outside
  
  if(pos2[0]<0||pos2[1]<0||pos2[2]<0||
     pos2[0]>=ref->xdim||pos2[1]>=ref->ydim||pos2[2]>=ref->zdim){
     //pos2[0]>=in->xdim||pos2[1]>=in->ydim||pos2[2]>=in->zdim){
  ...
  vec[0]=ref->vec[ind2][0]; 
  vec[1]=ref->vec[ind2][1]; 
  vec[2]=ref->vec[ind2][2]; 

  rot_cd(vec,vec2,mtx);

  out->dens[ind]=ref->dens[ind2];
  out->vec[ind][0]=vec2[0];
  out->vec[ind][1]=vec2[1];
  out->vec[ind][2]=vec2[2];
...
void PrintTbl(TBL *TopTbl,int i,MRC *m1,MRC *m2,float Ave,float Std){
 int f[3];
 float t[3],cent[3];
 float mtx[3][3];
...
bool SearchMAPfftMT(MRC *m1,MRC *m2,MRC *std2,int mode, bool Emode, MRC *com1,MRC *com2){
 ...
 fftwf_plan px12=NULL;
 int trans[3];

 //if(px1)
  fftwf_destroy_plan(px1);
 //if(px12)
  fftwf_destroy_plan(px12);
...
/*
 if(Emode==true){
  ...
  //RotMRCbyQ2(m2,std2,&MT_mtmp[0],TopTbl[i].q);
  //Bestscore=GetScore2(m1,&MT_mtmp[0],TopTbl[i].t,sco);
  float Bestscore=GetScore2(com1,com2,t,sco);
  printf("sco=%f\n",sco[0]);
  //TopTbl[i].sco=sco[0];
  //PrintTbl(TopTbl,i,m1,m2,0,1.0);
  return 0;
 }
*/
 ...
 for(int i=0;i<Nth;i++){
  ...
  if((MT_mtmp[i].sco=(float *)calloc(sizeof(float),xyzdim))==NULL)
   return true;
  if((MT_mtmp[i].vec=(float **)malloc(sizeof(float *)*xyzdim))==NULL)
   return true;
  for(int j=0;j<xyzdim;j++)
   if((MT_mtmp[i].vec[j]=(float *)malloc(sizeof(float)*3))==NULL)
    return true;
 ...
 float rd3=1.000/(float)d3;

 px1=fftwf_plan_dft_r2c_3d(n,n,n,x1,X1,FFTW_ESTIMATE); 
 //px1=fftwf_plan_dft_r2c_3d(n,n,n,x1,X1,FFTW_PATIENT); 
 //for(int i=0;i<xyzdim;i++) x1[i]=1.00;
 //for(int i=0;i<xyzdim;i++) x2[i]=1.00;
  ...
  px12=fftwf_plan_dft_c2r_3d(n,n,n,X12,x12,FFTW_ESTIMATE);
  Complex_Complex(X1,X2,X12,xyzdim);
  fftwf_execute_dft_c2r(px12,X12,x12);
 float tmp_sco=FindBestTrans(x12,x12,x12,m1->xdim,trans);
 ...
 for(int job=0;job<Njobs;job++){
  ...
  RotMRCbyQ2(m2,std2,mtmp,tbl[job].q);
 ...
 if(mode==3){
  for(int i=0;i<TopN;i++){
   ...
   RotMRCbyQ2(m2,std2,&MT_mtmp[0],tbl[i].q);
  ...
  if(Emode==true){
   ...
   //RotMRCbyQ(m2,&MT_mtmp[0],tbl[i].q);
   RotMRCbyQ2(m2,std2,&MT_mtmp[0],tbl[i].q);
   float Bestscore=GetScore2(m1,&MT_mtmp[0],tbl[i].t,sco);
   tbl[i].sco=sco[0];
   PrintTbl(tbl,i,m1,m2,Ave,Std);
   if(cmd.ShowGrid)
    ShowVec3(m1,&MT_mtmp[0],tbl[i].t);
...
*/
  ...
  RotMRCbyQ2(m2,std2,mtmp,TopTbl[job].q);
...
*/
 ...
 if(Emode==true){
  ...
  //RotMRCbyQ2(m2,std2,&MT_mtmp[0],TopTbl[i].q);
  //Bestscore=GetScore2(m1,&MT_mtmp[0],TopTbl[i].t,sco);
  Bestscore=GetScore2(com1,com2,TopTbl[i].t,sco);
  TopTbl[i].sco=sco[0];
  PrintTbl(TopTbl,i,com1,com2,Ave,Std);
  if(cmd.ShowGrid)
   ShowVec3(m1,&MT_mtmp[0],TopTbl[i].t);
 }
...
bool SimpleScoring(MRC *com1,MRC *com2){

  int i=0;
  float sco[5];
  TBL *TopTbl;
  if((TopTbl=(TBL *)malloc(sizeof(TBL)*2))==NULL)
   return true;
  ...
  float Bestscore=GetScore2(com1,com2,TopTbl[i].t,sco);
  TopTbl[i].sco=sco[0];
  PrintTbl(TopTbl,i,com1,com2,1.0,1.0);
...
bool SearchMAPfftMT_OVCC(MRC *m1,MRC *m2,int mode, int Amode, bool Emode){
 ...
 float q[4];
  ...
  printf("MTX: %.3f %.3f %.3f ",tmp_mtx[0][0],tmp_mtx[0][1],tmp_mtx[0][2]);
  printf("%.3f %.3f %.3f ",tmp_mtx[1][0],tmp_mtx[1][1],tmp_mtx[1][2]);
  printf("%.3f %.3f %.3f\n",tmp_mtx[2][0],tmp_mtx[2][1],tmp_mtx[2][2]);
 }
 return 0;
*/
 ...
 float rstd2=1.000/((m1->std*m2->std));
 float rstd3=1.000/((m1->std_norm_ave*m2->std_norm_ave));
...
float GetScore(MRC *m1, MRC *m2,int T[3]){
 ...
 float cc_sum=0;
 float pcc_sum=0;
 float std1,std2,d1,d2;
 float pstd1,pstd2,pd1,pd2;
 std1=std2=0;
 int Ncc=0;
 //int hist[20];
 ...
 //std1=sqrt(std1);
 //std2=sqrt(std2);
 printf("Overlap= %.4f %d/%d CC= %f PCC= %f ",(float)Nm/(float)tot,Nm,tot,cc_sum/(std1*std2),
                 pcc_sum/(pstd1*pstd2));
 printf("Score= %.1f\n",sco);
 return sco;
}

float GetScore2(MRC *m1, MRC *m2,int T[3],float results[5]){
 ...
 for(int z=0;z<m1->zdim;z++){
  ...
  ind2=xydim*pz+xdim*py+px;
  //printf("%d %d %d / %d %d %d idx= %d %d\n",x,y,z,m2->xdim,m2->ydim,m2->zdim,ind1,ind2);
	...
	lap2=-6.00*d2;
	//x+1,y,z
	if(x+1<m1->xdim) 	lap1+=m1->dens[xydim*z +xdim*y +x+1];
	if(px+1<m1->xdim) 	lap2+=m2->dens[xydim*pz+xdim*py+px+1];
	//x-1,y,z
	if(x-1>=0) 	lap1+=m1->dens[xydim*z +xdim*y +x-1];
	if(px-1>=0) 	lap2+=m2->dens[xydim*pz+xdim*py+px-1];
	//x,y+1,z
	if(y+1<m1->ydim) 	lap1+=m1->dens[xydim*z +xdim*(y+1) +x];
	if(py+1<m1->ydim) 	lap2+=m2->dens[xydim*pz+xdim*(py+1)+px];
	//x,y-1,z
	if(y-1>=0) 	lap1+=m1->dens[xydim*z +xdim*(y-1) +x];
	if(py-1>=0) 	lap2+=m2->dens[xydim*pz+xdim*(py-1)+px];
	//x,y,z+1
	if(z+1<m1->zdim) 	lap1+=m1->dens[xydim*(z+1) +xdim*y +x];
	if(pz+1<m1->zdim) 	lap2+=m2->dens[xydim*(pz+1)+xdim*py+px];
	//x,y,z-1
 ...
 //std1=sqrt(std1);
 //std2=sqrt(std2);
 printf("Overlap= %.4f %d/%d CC= %f PCC= %f LAP= %f ",(float)Nm/(float)tot,Nm,tot,cc_sum/(std1*std2),
		 pcc_sum/(pstd1*pstd2),lap_sum);
 printf("Score= %.1f\n",sco);
 results[0]=sco;//dot score
 results[1]=(float)Nm;//Overlap
