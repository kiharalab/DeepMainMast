...
bool CopyMrc(MRC *in,MRC *out){
	...
	out->my = in->my;
	out->mz = in->mz;
	out->widthx = in->widthx;
	out->widthy = in->widthy;
	out->widthz = in->widthz;
	out->xdim = in->xdim;
	out->ydim = in->ydim;
	out->zdim = in->zdim;
	out->orgxyz[0] = in->orgxyz[0];
	out->orgxyz[1] = in->orgxyz[1];
	out->orgxyz[2] = in->orgxyz[2];
	out->NumVoxels=in->NumVoxels;
 	if((out->dens=(float*)malloc(sizeof(float)*out->NumVoxels))==NULL)
 	 return true;
	for(int i=0;i<out->NumVoxels;i++)
	 out->dens[i]=in->dens[i];
...
*/
 ...
 for(int i=0;i<Nmodel;i++){
	...
	//printf("MODEL %d\n",i);
	//check loop again
	for(int j=0;j<fg->Nnode-1;j++){
		for(int k=j+1;k<fg->Nnode;k++){
		 if(fg->n_path[j]==fg->n_path[k])
	 	 	printf("Hidden loop %d\n",fg->n_path[j]);
			Ntabu++;
		}
	}
...
bool NormalizeMrc(MRC *mrc,float Dcut){
 int xydim = mrc->xdim*mrc->ydim;
 int xdim = mrc->xdim;
 int pos[3];
 ...
 for(int x=0;x<mrc->xdim*mrc->ydim*mrc->zdim;x++){
  if(mrc->dens[x] < Dcut){
   ...
   if(mrc->dens[x]>Dmax)
    Dmax=mrc->dens[x];
   if(mrc->dens[x]<Dmin && mrc->dens[x] >0.00)
    Dmin=mrc->dens[x];
  }
 }
 //Normalize Nov3 2021
 printf("#Normalizing....Min Density:%f Max Density:%f\n",Dmin,Dmax);
 float Rnorm=1.000/(Dmax-Dmin);
 for(int x=0;x<mrc->xdim*mrc->ydim*mrc->zdim;x++){
  if(mrc->dens[x]>0)
   mrc->dens[x]=Rnorm*(mrc->dens[x]-Dmin);
 }
 return false;
}
...
//Probability+MRC Weight_density and Weight_ca
bool AddProbToMrc(PTBL *tbl,MRC *mrc,float weight, float Wca,float Dcut,int mode){
 int xydim = mrc->xdim*mrc->ydim;
 int xdim = mrc->xdim;
 int pos[3];
 ...
 for(int i=0;i<tbl->N;i++){
  ...
  if(mrc->dens[ind] < 0.00)//Mask by Density
   continue;
  //23rd- Other,N,CA,C,O,CB
  if(mode == 0)//Main-chain N, CA, C atom
   mrc->dens[ind]=(tbl->prob[i][24]+tbl->prob[i][25]+tbl->prob[i][26]);
  if(mode == 1)//Density + CA Mode
   mrc->dens[ind]=weight*mrc->dens[ind]+Wca*tbl->prob[i][25];
  if(mode == 2)//W*Density + W*CA + N + C Mode
   mrc->dens[ind]=weight*mrc->dens[ind]+Wca*tbl->prob[i][25] + tbl->prob[i][24]+tbl->prob[i][26];
 }
 for(int x=0;x<mrc->xdim*mrc->ydim*mrc->zdim;x++){
  if(mrc->dens[x] < 0.00)
...
bool AssignUnetProbToNode(MRC *ATOM_mrc,MRC *AA_mrc,GRAPH *g){
 PTBL *tbl;
 MRC *mrc = &AA_mrc[0];
 float dcut = 3.0 / mrc->widthx;//3A cut-off
 float dcut2 = dcut*dcut;
 int MaxIdx=mrc->xdim*mrc->ydim*mrc->zdim;
 ...
 for(int i=0;i<g->Nnode;i++){
  ...
  int round_cd[3];
  //round_cd[0] = round(pt->cd[i][0]);
  round_cd[0] = round((g->node[i].real_cd[0]-mrc->orgxyz[0])/mrc->widthx);
  round_cd[1] = round((g->node[i].real_cd[1]-mrc->orgxyz[1])/mrc->widthx);
  round_cd[2] = round((g->node[i].real_cd[2]-mrc->orgxyz[2])/mrc->widthx);


  //printf("%f %f %f\n",pt->cd[i][0],pt->cd[i][1],pt->cd[i][2]);
  idx = mrc->xdim*mrc->ydim*round_cd[2] + mrc->xdim*round_cd[1] + round_cd[0];
  //printf("%d %d %d idx= %d/%d\n",round_cd[0],round_cd[1],round_cd[2],idx,mrc->xdim*mrc->ydim*mrc->zdim)

	//map region check
	bool inside=true;
	if(round_cd[0]<0||round_cd[1]<0||round_cd[2]<0)
	 inside=false;
	else if(round_cd[0]>mrc->xdim-1||round_cd[1]>mrc->ydim-1||round_cd[2]>mrc->zdim-1)
	 inside=false;


        if(idx==-1 || inside==false){//No Hits
       ...
       //AA
        ...
        for(int j=0;j<20;j++){
         //printf("AAP= %f\n",AA_mrc[j].dens[idx]);
         g->node[i].AAP[j]=AA_mrc[j].dens[idx];
        }

        for(int j=0;j<6;j++){//Other,N,CA,C,O,CB
         //printf("ATMP= %f\n",AA_mrc[j].dens[idx]);
         g->node[i].ATOMP[j]=ATOM_mrc[j].dens[idx];
        }
 ...
 //Reference P
 for(int j=0;j<20;j++) REFaa[j]=SumPaa[j]/TotalPaa;
 for(int j=0;j<6;j++) REFatm[j]=SumPatm[j]/TotalPatm;
 //for(int j=0;j<3;j++) REFss[j]=SumPss[j]/TotalPss;
 ...
 for(int i=0;i<g->Nnode;i++){
        for(int j=0;j<20;j++)
	 if(g->node[i].AAP[j]==0.0)//avoid infinit
	  g->node[i].LogAA[j]=0.0;
	 else
          g->node[i].LogAA[j]=log(g->node[i].AAP[j]/REFaa[j]);
        for(int j=0;j<6;j++)//23rd- Other,N,CA,C,O,CB
         if(g->node[i].ATOMP[j]==0.0)
	  g->node[i].LogATOM[j]=0.0;
	 else
          g->node[i].LogATOM[j]=log(g->node[i].ATOMP[j]/REFatm[j]);
        //for(int j=0;j<3;j++)
        // g->node[i].LogSS[j]=log(g->node[i].SSP[j]/REFss[j]);
 }
...
bool AssignProbToNodeSimple(PTBL *tbl,GRAPH *g,MRC *mrc){
 float dcut = 3.0 / mrc->widthx;//3A cut-off
 float dcut2 = dcut*dcut;
 //search closest points
 ...
 for(int i=0;i<g->Nnode;i++){
  ...
  g->node[i].id=i;
	 ...
	 //realcd
	 float real_cd[3];
	 real_cd[0] = tbl->prob[j][0] * mrc->widthx+mrc->orgxyz[0];
	 real_cd[1] = tbl->prob[j][1] * mrc->widthx+mrc->orgxyz[1];
	 real_cd[2] = tbl->prob[j][2] * mrc->widthx+mrc->orgxyz[2];

	 dist = 
		(g->node[i].real_cd[0] - real_cd[0])*(g->node[i].real_cd[0] - real_cd[0])+
		(g->node[i].real_cd[1] - real_cd[1])*(g->node[i].real_cd[1] - real_cd[1])+
		(g->node[i].real_cd[2] - real_cd[2])*(g->node[i].real_cd[2] - real_cd[2]);
	 if(dist > dcut2)
	  continue;
	 if(MinDist > dist){
	  MinDist=dist;
...
bool ScoreFrag(FRAG_DB *db,int Ndb, SEQFG *sfg, int Nsfg,SEQ_NODE *res){
 ...
 for(int i=0;i<Nsfg;i++){
  if((res[i].score=(float*)malloc(sizeof(float)*Ndb))==NULL)
   return true;
  if((res[i].raw_score=(float*)malloc(sizeof(float)*Ndb))==NULL)
   return true;
  if((res[i].rmsd=(float*)malloc(sizeof(float)*Ndb))==NULL)
   return true;
  if((res[i].ali=(NODE ***)malloc(sizeof(NODE **)*Ndb))==NULL)
   return true;
	for(int j=0;j<Ndb;j++)
  	 //if((res[i].ali[j]=(NODE **)malloc(sizeof(NODE *)*sfg[j].l))==NULL)
  	 if((res[i].ali[j]=(NODE **)malloc(sizeof(NODE *)*100))==NULL)
   	  return true;
 }
 ...
 #pragma omp parallel for schedule(dynamic,10)
 for(int s=0;s<Nsfg;s++){//Each Sequence...
  float sco=0;
  float maxsco;
  int N=0;
  NODE *tmp_ali[100];
 	for(int i=0;i<Ndb;i++){//Each Node
  	 if(db[i].N==0) continue;
	 //compute....
	 maxsco=0.00;
		for(int j=0;j<db[i].N;j++){//Each str fragment
	...
	for(int i=0;i<N-1;i++){
	 	for(int p=0;p<sfg[s].l;p++){
		  if(res[s].ali[i][p]==NULL)
		   continue;
		  for(int q=p+1;q<sfg[s].l;q++){
		   if(res[s].ali[i][q]==NULL)
		    continue;
		   if(res[s].ali[i][p]==res[s].ali[i][q])
		    printf("***Same Node!? %d %d %d\n",s,i,p);
		  }
	 	 }
	...
	for(int i=0;i<N;i++){
	 ave+=res[s].score[i];
	 res[s].raw_score[i]=res[s].score[i];
	}
	ave=ave/(float)N;
	for(int i=0;i<N;i++){
 ...
 for(int i=0;i<s->l;i++){
  ...
  for(int j=i+1;j<s->l;j++){
   ...
   if(ali2[i]==ali2[j])
    printf("##Loop %d %d\n",i,j);
   if (
	(out[i]->real_cd[0] - out[j]->real_cd[0])*(out[i]->real_cd[0] - out[j]->real_cd[0]) < 0.01 &&
	(out[i]->real_cd[1] - out[j]->real_cd[1])*(out[i]->real_cd[1] - out[j]->real_cd[1]) < 0.01 &&
	(out[i]->real_cd[2] - out[j]->real_cd[2])*(out[i]->real_cd[2] - out[j]->real_cd[2]) < 0.01 
	)
    printf("##LoopCD %d %d\n",i,j);
  }
 }
