	...
	while(fgets(line,LIN,fpin)){ 
		if(!strncmp(line,"ATOM",4) 
		 ...
		 ||!strncmp(&line[16]," ",1))){
			...
			pdb->TypeAtomId[i]=Atom2Int(pdb->TypeAtom[i]);
			//Only CA		
...
int side_det(PDB *pdb,int num){
 ...
 if(!strncmp(pdb->TypeAtom[num],"OXT",3))
  return(4);
 if(!strncmp(pdb->TypeAtom[num],"CB ",3))
  return(5);
...
*/
 if(!strncmp(TypeAtom,"N  ",3))
  return(1);
 if(!strncmp(TypeAtom,"CA ",3))
  return(2);
 if(!strncmp(TypeAtom,"C  ",3))
  return(3);
 if(!strncmp(TypeAtom,"O  ",3))
  return(4);
 if(!strncmp(TypeAtom,"OXT",3))
  return(4);
 if(!strncmp(TypeAtom,"CB ",3))
  return(5);
