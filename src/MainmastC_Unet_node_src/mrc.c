...
bool meanshift(MRC *m,POINTS *p,float map_t){
 int i,j,k,ind;
 int cnt=0;
 int xydim=m->xdim*m->ydim;
...
bool AssignUnetProbToNode(MRC *ATOM_mrc,MRC *AA_mrc,POINTS *pt,GRAPH *g){
 PTBL *tbl;
 MRC *mrc = &AA_mrc[0];
 float dcut = 3.0 / mrc->widthx;//3A cut-off
 float dcut2 = dcut*dcut;

 if((g->node=(NODE*)malloc(sizeof(NODE)*pt->Ncd))==NULL)
  return true;
 ...
 for(int i=0;i<pt->Ncd;i++){
  ...
  round_cd[1] = round(pt->cd[i][1]);
  round_cd[2] = round(pt->cd[i][2]);
 ...
 for(int s=0;s<Nsfg;s++){//Each Sequence...
  ...
  NODE *tmp_ali[100];
	 ...
	 if(maxsco>0.00){
		/*
	  	for(int p=0;p<sfg[s].l;p++){
		  if(res[s].ali[N][p]==NULL)
		   continue;
		  for(int q=p+1;q<sfg[s].l;q++){
		   if(res[s].ali[N][q]==NULL)
		    continue;
		   //printf("%d vs %d\n",p,q);
		   if(res[s].ali[N][p]==res[s].ali[N][q])
		    printf("xxxSame Node!? %d %d %d\n",s,N,p);
		  }
	 	 }
		*/
