...
def get_args(): 
    ...
    parser.add_argument("LIST", type=str, help="NODE file (in PDB format)")
    ...
    parser.add_argument('--OutPath', type=str, dest='OutNamePath', default='path.pdb', help='Final Outpt PDB file')
...
def print_solution_pdb(data,p,outname):
...
def VRP_solver(data):
    ...
    #search_parameters.time_limit.seconds = args.SecTrace
...
def main():
    ...
    print_solution_pdb(ALL_PATH,data['node'],args.OutNamePath)
    ...
    #for path in ALL_PATH+Merged_Path:
    for path in ALL_PATH: #######
        if len(path) < 4:#Too short
            continue
        
        #for Wdist in [-0.1, -0.5]:
        for Wdist in [0.0]:
            dmtx = data['real_d_matrix']
            smtx = data['score_matrix']
            ref_smtx = data['score_matrix_nonegative']
            AL=SimpleDP(path,dmtx,smtx,ref_smtx,0,Niter,Wdist) #same order
            ALL_ALI.append(AL)
            AL=SimpleDP(path,dmtx,smtx,ref_smtx,1,Niter,Wdist) #reverse
            ALL_ALI.append(AL)
 
            dmtx = data['real_d_matrix']
            smtx = data['score_matrix_nonegative']
            AL=SimpleDP(path,dmtx,smtx,ref_smtx,0,Niter,Wdist) #same order
            ALL_ALI.append(AL)
            AL=SimpleDP(path,dmtx,smtx,ref_smtx,1,Niter,Wdist) #reverse
            ALL_ALI.append(AL)
    ...
    for alis in ALL_ALI:
        for a in alis:
            #print(a['score'],a['alignment'])
            if(len(a['alignment'])>3):
                FRAG_TBL.append(a)

    print('Total Fragment:',len(FRAG_TBL))
    dmtx = data['real_d_matrix']
    CLASH=ClashCheck(FRAG_TBL,dmtx,data['sequence']) #Consider chain-id
    #Combine? Assemble?
    # Model
    ...
    objective_terms = []
    for i in range(Nfrag):
        objective_terms.append(int(FRAG_TBL[i]['score']) * x[i])
        #print(smtx[i][j])
    ...
    #solver.parameters.max_time_in_seconds = args.SecCp
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        for i in range(Nfrag):
            if solver.BooleanValue(x[i]):
                print(f'Fragment {i} assigned to use Score = {FRAG_TBL[i]}')
                results.append(FRAG_TBL[i])
    else:
        print('No solution found.')
    
    ShowPDB_fragments(results,data,args.OutNameThread)
