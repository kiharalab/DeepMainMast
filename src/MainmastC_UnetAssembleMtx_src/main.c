 ...
 for(int i=0;i<APDB.NumOfAtom - cmd.FragLen ;i++){
  ...
  if(APDB.TypeResId[order]!=seq.seq_code[ResNum-1])
   continue;
	...
	tmp_model.Rmsd=0;
	tmp_model.Len=Ncd;
	//for(int j=0;j<cmd.FragLen;j++)
	// printf("%f %d %d\n",tmp_model.Ca[j].real_cd[0],APDB.TypeResId[order],seq.seq_code[ResNum-1]);
 ...
 for(int m1=0;m1<Nmodel;m1++){
  ...
  for(int m2=0;m2<Nmodel;m2++){
	...
	if(CheckConf(model[m1],model[m2],&seq)==true)
	 printf("%d,",m2);
	else if(cmd.Cmode==true && CheckContact(model[m1],model[m2],CONTtbl,seq.len)==true)
	 printf("%d,",m2);
  }
   puts("");
 }
