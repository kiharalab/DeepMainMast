...
typedef struct{
 NODE Ca[5000];//For Big Fragments
 int Start,End,Len;
 float RawScore,Score,Rmsd;
 int id;
 int Cid,Fid;//Chain and Fragment IDs
} RESULT_MODEL;
...
bool CheckConfFrag(RESULT_MODEL *,RESULT_MODEL *,SEQ *,bool);
bool CheckHomoShape(RESULT_MODEL *,RESULT_MODEL *,SEQ *,float);
bool CheckHomSeq(RESULT_MODEL *,RESULT_MODEL *,SEQ *);
bool CheckPairs(RESULT_MODEL *,RESULT_MODEL *,RESULT_MODEL *,RESULT_MODEL *,SEQ *,float);
int CheckPairsDetail(RESULT_MODEL *,RESULT_MODEL *,RESULT_MODEL *,RESULT_MODEL *,SEQ *,float);
bool CheckSameGroup(int,int,SEQ*);
 ...
 for(int i=0;i<APDB.NumOfAtom - cmd.FragLen ;i++){
  ...
  if(APDB.TypeResId[order]!=seq.seq_code[ResNum-1])
   continue;
	...
	//Check fragment
	int Ncd=0;
	for(int j=0;i+j < APDB.NumOfAtom ;j++){
	 //Check Residue Number
	 if(ResNum + j != APDB.ResNum[i+j]) //Gap
	  break;
	 //Distance Gap
	 if(j>0){
	  float d2=(APDB.xyz[i+j][0]-APDB.xyz[i+j-1][0])*(APDB.xyz[i+j][0]-APDB.xyz[i+j-1][0])+
	  (APDB.xyz[i+j][1]-APDB.xyz[i+j-1][1])*(APDB.xyz[i+j][1]-APDB.xyz[i+j-1][1])+
	  (APDB.xyz[i+j][2]-APDB.xyz[i+j-1][2])*(APDB.xyz[i+j][2]-APDB.xyz[i+j-1][2]);
	  if(d2 > cmd.CACAdist*cmd.CACAdist)
	   break;
	 }
	  //Check Chain ID
	 if(seq.chain_id[ResNum-1]!=seq.chain_id[ResNum-1+j])
	  break;
	...
	tmp_model.End=tmp_model.Start + Ncd;
	tmp_model.Score=0;//Not yet
	tmp_model.RawScore=0;//not yet
	tmp_model.Rmsd=0;
	tmp_model.Len=Ncd;
	tmp_model.Cid = seq.chain_id[tmp_model.Start];
	tmp_model.Fid = Nmodel;
	Nmodel=AddModel(&tmp_model,model,Nmodel);
	//printf("Num of Coords: %d Start %d\n",Ncd,tmp_model.Start);
	//printf("CD %.3f %.3f %.3f\n",tmp_model.Ca[0].real_cd[0],tmp_model.Ca[0].real_cd[1],tmp_model.Ca[0].real_cd[2]);
	//Update i
	i=i+Ncd-1;
	...
	//Local Sequence Based
/*
	for(int pos=0;pos<seq.len-1;pos++){
...
*/
 ...
 //Homo oligomer
 int Nfragment = Nmodel;//Original Fragment
 for(int i=0;i<Nfragment;i++){
	printf("FID %d CID %d START %d\n",model[i]->Fid,model[i]->Cid,model[i]->Start);
	//Check
	for(int j=0;j<seq.Nhom_pairs;j++){
		int new_cid=-1;
	 	if(seq.hom_chain_list[j][0]==model[i]->Cid){
		 new_cid = seq.hom_chain_list[j][1];
		}
		if(seq.hom_chain_list[j][1]==model[i]->Cid){
		 new_cid = seq.hom_chain_list[j][0];
		}
		//Add new cid
		if(new_cid == -1) continue;
		printf("Add new CID %d\n",new_cid);

		int pos = model[i]->Start - seq.chain_start[model[i]->Cid];//position in the chain
		int new_start = seq.chain_start[new_cid] + pos;
		tmp_model.Start=new_start;
		tmp_model.End=tmp_model.Start + model[i]->Len;
		tmp_model.Score=0;//Not yet
		tmp_model.RawScore=0;//not yet
		tmp_model.Rmsd=0;
		tmp_model.Len=model[i]->Len;
		tmp_model.Cid = new_cid;
		tmp_model.Fid = model[i]->Fid;
			for(int k=0;k<tmp_model.Len;k++){
			 tmp_model.Ca[k].real_cd[0]=model[i]->Ca[k].real_cd[0];
			 tmp_model.Ca[k].real_cd[1]=model[i]->Ca[k].real_cd[1];
			 tmp_model.Ca[k].real_cd[2]=model[i]->Ca[k].real_cd[2];
	 		 tmp_model.Ca[k].node_id = -1;
			}
		Nmodel=AddModel(&tmp_model,model,Nmodel);
	}
 }
 ...
 printf("#Nmodel= %d\n",Nmodel);
 //return 0;
 if((g.node=(NODE*)malloc(sizeof(NODE)*Nmodel*APDB.NumOfAtom))==NULL)
  return 0;
 g.Nnode=0;
 ...
 for(int m=0;m<Nmodel;m++){
  ...
  printf("FRG %d FID %d CID %d RAWSCO= %f ZSCORE= %f RMSD= %.3f ALI= ",
  Nmodel2,model[m]->Fid,model[m]->Cid,model[m]->RawScore,model[m]->Score,model[m]->Rmsd);
 ...
 puts("#DONE FRG");
 //check matrix for only Original Fragmens
 for(int m1=0;m1<Nfragment;m1++){
  printf("MTX_SAME %d ",m1);//Same chain
  for(int m2=0;m2<Nfragment;m2++){
	if (m1==m2)
	 continue;
	if (CheckSameGroup(model[m1]->Cid,model[m2]->Cid,&seq)==false)
	 continue;
	//Same chain?
	if(CheckConfFrag(model[m1],model[m2],&seq,true)==true){
	 printf("%d,",m2);
	 continue;
	}
  }
  puts("");
  printf("MTX_DIFF %d ",m1);//Different chain
  for(int m2=0;m2<Nfragment;m2++){
	if (m1==m2)
	 continue;
	//Same chain?
	if(CheckConfFrag(model[m1],model[m2],&seq,false)==true){
	 printf("%d,",m2);
	 continue;
	}
  }
 ...
 if(cmd.Hmode==true){
 //if(false){
  printf("##Homockecking Nfragment= %d\n",Nfragment);

  RESULT_MODEL **pair_list;
  int Npair=0;
  if((pair_list=(RESULT_MODEL **)malloc(sizeof(RESULT_MODEL *)*(int)(Nfragment)*(int)(Nfragment)))==NULL)
   return false;



  for(int m1=0;m1<Nfragment;m1++)
   model[m1]->id=m1;//init id
  for(int m1=0;m1<Nfragment;m1++){
   int chid1=model[m1]->Cid;
   	for(int m2=m1+1;m2<Nfragment;m2++){
   	 int chid2=model[m2]->Cid;
   	 //printf("cid %d %d -> %d %d\n",m1,m2,chid1,chid2);

	 if (CheckSameGroup(chid1,chid2,&seq)==false)//Same group
	  continue;

   	 if(CheckConfFrag(model[m1],model[m2],&seq,true)==true)//Same chain
   	  continue;
	 
	 pair_list[2*Npair]=model[m1];
	 pair_list[2*Npair+1]=model[m2];
	 //printf("pair%d %d %d\n",Npair,model[m1]->id,model[m2]->id);
    	 Npair++;
	}
  }
  printf("##Npair= %d\n",Npair);
  ...
  for(int pr1=0;pr1<Npair;pr1++){
   ...
   f12 = pair_list[2*pr1+1];
	 ...
	 if(CheckSameGroup(f11->Cid,f21->Cid,&seq)==false)//Same homology group
	  continue;

	 int Nchk=CheckPairsDetail(f11,f12,f21,f22,&seq,cmd.RMSD);
	 if(Nchk>0){
	  //Remove conflicting pairs
		 if(CheckConfFrag(f11,f21,&seq,false)==true)
		   continue;
		 if(CheckConfFrag(f11,f22,&seq,false)==true)
		   continue;
		 if(CheckConfFrag(f12,f21,&seq,false)==true)
		   continue;
		 if(CheckConfFrag(f12,f22,&seq,false)==true)
		   continue;
	  printf("PAIR %d,%d %d,%d %d\n",f11->id,f12->id,f21->id,f22->id,Nchk);
	 }
	}
  }
/*
	 ...
	 bool hchk=false;
		for(int m2=0;m2<m1;m2++){
		 if(CheckHomSeq(model[m1],model[m2],&seq)==true){
		  //printf("HOM %d %d\n",model[m1]->id,model[m2]->id);
		  hchk=true;
		 }
		}
		if(hchk==true){
	 	...
	 	printf("HOM %d",model[m1]->id);
  		for(int m2=m1+1;m2<Nmodel2;m2++){
		 if(CheckHomSeq(model[m1],model[m2],&seq)==true){
		  printf(" %d",model[m2]->id);
		 }
...
bool CheckSameGroup(int cid1,int cid2,SEQ *seq){
 if(cid1==cid2)
  return true;
 for(int i=0;i<seq->Nhom_pairs;i++){
  if(cid1 == seq->hom_chain_list[i][0] && cid2 == seq->hom_chain_list[i][1])
   return true;
  if(cid2 == seq->hom_chain_list[i][0] && cid1 == seq->hom_chain_list[i][1])
   return true;
 }
 return false;
}

bool CheckHomSeq(RESULT_MODEL *Fa,RESULT_MODEL *Fb, SEQ *seq){
 int chid_a = seq->chain_id[Fa->Start];
 int chid_b = seq->chain_id[Fb->Start];

 int pos_a=Fa->Start - seq->chain_start[chid_a];//position within the chain
 int pos_b=Fb->Start - seq->chain_start[chid_b];
 ...
 bool chk=false;
 for(int i=0;i<seq->Nhom_pairs;i++){
	if(seq->hom_chain_list[i][0]==chid_a && seq->hom_chain_list[i][1]==chid_b){
	 chk=true;
	 break;
	}
	if(seq->hom_chain_list[i][0]==chid_b && seq->hom_chain_list[i][1]==chid_a){
	 chk=true;
	 break;
	}
 }
 if (chk==false)//Not homo oligomer
  return false;

 //Coordinates
 float dist;
 for(int i=0;i<Fa->Len;i++)
  dist=Dist2(&(Fa->Ca[i]),&(Fb->Ca[i]));
  if(dist > 0.01)
   return false;
 return true;
}
...
//Check Inconsistent positions
int CheckPairsDetail(RESULT_MODEL *Fa,RESULT_MODEL *Fb,RESULT_MODEL *Fc,RESULT_MODEL *Fd, SEQ *seq, float rcut){
	
	float r2 = rcut*rcut;
	//int chid_ab = seq->chain_id[Fa->Start];
	int chid_a = seq->chain_id[Fa->Start];
	int chid_b = seq->chain_id[Fb->Start];
	//int chid_cd = seq->chain_id[Fc->Start];
	int chid_c = seq->chain_id[Fc->Start];
	int chid_d = seq->chain_id[Fd->Start];
	/*
	int pos_a=Fa->Start - seq->chain_start[chid_ab];//position within the chain
	int pos_b=Fb->Start - seq->chain_start[chid_ab];
	int pos_c=Fc->Start - seq->chain_start[chid_cd];
	int pos_d=Fd->Start - seq->chain_start[chid_cd];
	*/
	int pos_a=Fa->Start - seq->chain_start[chid_a];//position within the chain
	int pos_b=Fb->Start - seq->chain_start[chid_b];
	int pos_c=Fc->Start - seq->chain_start[chid_c];
	int pos_d=Fd->Start - seq->chain_start[chid_d];
	...
	//a vs c
	for(int i=0;i<Fa->Len;i++){
        int Pa=pos_a+i;//position in the chain
	...
	//printf("COMM Nac= %d Nad= %d\n",Ncom_ac,Ncom_ad);
	if(Ncom_ac==0 && Ncom_ad==0){
		return 0;
	}
	int Ncom_bc=0;
	int Ncom_bd=0;
	...
	for(int i=0;i<Fb->Len;i++){
        int Pb=pos_b+i;//position in the chain
        for(int j=0;j<Fc->Len;j++){
	        int Pc=pos_c+j;
	        if(Pb!=Pc){
		        continue;
	...
	//printf("COMM Nbc= %d Nbd= %d\n",Ncom_bc,Ncom_bd);
	if(Ncom_bc==0 && Ncom_bd==0){
		return 0;
	}
	//Fa-Fb vs Fc-Fd
	int p_a,p_b,p_c,p_d;
	float d_ab,d_cd;
	if(Ncom_ac > Ncom_ad && Ncom_bd > Ncom_bc){
		for(int i=0;i<Ncom_ac;i++){
			p_a=tbl_ac[i][0];
			p_c=tbl_ac[i][1];
			for(int j=0;j<Ncom_bd;j++){
				p_b=tbl_bd[j][0];
				p_d=tbl_bd[j][1];
				d_ab=Dist2(&(Fa->Ca[p_a]),&(Fb->Ca[p_b]));
				d_cd=Dist2(&(Fc->Ca[p_c]),&(Fd->Ca[p_d]));
				d_ab = sqrt(d_ab);
				d_cd = sqrt(d_cd);
				if((d_ab-d_cd)*(d_ab-d_cd)>r2){
				 //printf("D %d-%d %d-%d %f %f diff= %f\n",Fa->Start+p_a,Fb->Start+p_b,Fc->Start+p_c,Fd->Start+p_d,d_ab,d_cd,d_ab-d_cd);
				 Nbad++;
				}
			}
	...
	//Fa-Fb vs Fd-Fc
	if(Ncom_ac < Ncom_ad && Ncom_bd < Ncom_bc){
		for(int i=0;i<Ncom_ad;i++){
			p_a=tbl_ad[i][0];
			p_d=tbl_ad[i][1];
			for(int j=0;j<Ncom_bc;j++){
				p_b=tbl_bc[j][0];
				p_c=tbl_bc[j][1];
				d_ab=Dist2(&(Fa->Ca[p_a]),&(Fb->Ca[p_b]));
				d_cd=Dist2(&(Fc->Ca[p_c]),&(Fd->Ca[p_d]));
...
bool CheckConfFrag(RESULT_MODEL *f1,RESULT_MODEL *f2, SEQ *seq,bool flag){
 float dist2=0.0;
 ...
 if(flag==true){
  cid1 = f1->Cid;
  cid2 = f2->Cid;
  st1 = f1->Start - seq->chain_start[cid1]; //position in the chain
  ed1 = st1 + f1->Len;
  st2 = f2->Start - seq->chain_start[cid2];
  ed2 = st2 + f2->Len;
   ...
   return true;
 	...
 	for(int a=0;a<f1->Len;a++){
 		for(int b=0;b<f2->Len;b++){
  		  dist2=Dist2(&(f1->Ca[a]),&(f2->Ca[b]));
  		 //Overlap Check
  		 if(dist2 < ClashDist)//Clash Check
  		  return true;
  		}
 	}
 }
