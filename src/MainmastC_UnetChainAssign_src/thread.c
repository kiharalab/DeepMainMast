...
bool readseq(SEQ *seq,char *fname){
 ...
 if(ftype==0){
  ...
  for(i=0;i<seq->len;i++){
   if(seq->chain_start[seq->chain_id[i]]<0)
    seq->chain_start[seq->chain_id[i]]=i;
   seq->chain_len[seq->chain_id[i]]++;
  }
  for(int i=0;i<Nch;i++){
   printf("#ChainStart: %d\n",seq->chain_start[i]);
  }
  ...
  for(int i=0;i<Nch;i++){
	  for(int j=i+1;j<Nch;j++){
		  if(seq->chain_len[i]!=seq->chain_len[j])
		   continue;
		  bool hom_chk=true;
		  int st1=seq->chain_start[i];
		  int st2=seq->chain_start[j];
		  for(int k=0;k<seq->chain_len[i];k++){
			  //**********
			  if(seq->seq_code[k+st1]!=seq->seq_code[k+st2]){
				  hom_chk=false;
				  break;
			  }
		  ...
		  if(hom_chk==true){
			  seq->hom_chain_list[seq->Nhom_pairs][0]=i;
			  seq->hom_chain_list[seq->Nhom_pairs][1]=j;
			  seq->Nhom_pairs++;
		  }
	  }
  }
  for(int i=0;i<seq->Nhom_pairs;i++){
	  ...
	  printf("#Detected Homomer: %d chain %d = %d\n",i,seq->hom_chain_list[i][0],seq->hom_chain_list[i][1]);
  }
